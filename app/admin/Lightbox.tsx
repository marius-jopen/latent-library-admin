      ...
      >
        ...
        {(onPrev || onNext) ? (
          <div className="absolute inset-x-0 bottom-6 flex items-center justify-center gap-3">
            {onPrev ? (
              <button className="h-9 px-4 rounded-full bg-white/20 hover:bg-white/30 text-white" onClick={onPrev} aria-label="Previous">← Prev</button>
            ) : null}
            {onNext ? (
              <button className="h-9 px-4 rounded-full bg-white/20 hover:bg-white/30 text-white" onClick={onNext} aria-label="Next">Next →</button>
            ) : null}
          </div>
        ) : null}
      </div>
